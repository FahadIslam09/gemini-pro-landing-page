...
import { X, Copy, Check, Lock, Sparkles, Loader2, CheckCircle2, MessageCircle, ShieldCheck } from "lucide-react";
...
export default function CheckoutModal({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulated multi-stage asynchronous processing with feedback
    setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Card */}
      <div className="relative z-10 w-full max-w-lg bg-white rounded-[28px] shadow-2xl border border-brand-border p-6 sm:p-8 max-h-[92vh] overflow-y-auto animate-in zoom-in-95 duration-200">
        ...
        {!isSubmitting && (
          <button
            ...
            className="absolute top-5 right-5 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 text-brand-muted hover:text-brand-dark flex items-center justify-center transition-colors"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        {!isCompleted ? (
          <>
            ...
            <div className="grid grid-cols-3 gap-2 mb-5 bg-brand-surface p-1.5 rounded-2xl border border-brand-border">
              {[
                { id: "1m", label: "১ মাস", price: "৳১৪৯" },
                { id: "12m", label: "১২ মাস", price: "৳৩৯৯" },
                { id: "18m", label: "১৮ মাস (সেরা)", price: "৳৪৯৯" },
              ].map((p) => (
                <button
                  key={p.id}
                  type="button"
                  ...
                  className={`py-2 px-1 text-center rounded-xl transition-all ${
                    selectedPlan === p.id
                      ...
                      : "text-brand-body text-xs font-semibold hover:text-brand-dark"
                  }`}
                >
                  <span className="block text-xs">{p.label}</span>
                  <span className="block font-outfit text-xs font-bold text-brand-dark">
                    {p.price}
                  </span>
                </button>
              ))}
            ...
            {/* Plan Summary Banner */}
            <div className="bg-brand-subtle border border-brand-purple/20 rounded-2xl p-4 flex items-center justify-between mb-5">
              <div>
                <span className="font-bold text-sm text-brand-dark block">
                  {currentPlan.name}
                </span>
                <span className="text-xs text-brand-muted block">
                  Gemini 1.5 Pro + 2 TB Storage + YouTube Premium
                </span>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full h-11 px-3.5 bg-brand-surface border border-brand-border focus:border-brand-blue rounded-xl text-sm text-brand-dark outline-none transition-colors"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full h-11 px-3.5 bg-brand-surface border border-brand-border focus:border-brand-blue rounded-xl text-sm text-brand-dark outline-none transition-colors font-outfit"
                />
              </div>
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {[
                    ...
                    <button
                      ...
                      disabled={isSubmitting}
                      className={`h-11 rounded-xl border flex items-center justify-center font-bold text-sm transition-all ${
                        paymentMethod === m.id
              ...
              <div className="bg-brand-alt border border-dashed border-brand-border rounded-xl p-3.5 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="bg-white hover:bg-gray-100 text-brand-blue border border-brand-border rounded-md px-2 py-0.5 text-[11px] font-bold flex items-center gap-1 transition-colors"
                    >
              ...
              <div>
                ...
                <input
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full h-11 px-3.5 bg-brand-surface border border-brand-border focus:border-brand-blue rounded-xl text-sm font-mono text-brand-dark uppercase tracking-wider outline-none transition-colors"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full inline-flex items-center justify-center gap-2 bg-brand-gradient hover:bg-brand-gradient-hover text-white text-base font-semibold py-3.5 px-6 rounded-xl shadow-glow transition-all disabled:opacity-75 disabled:cursor-not-allowed hover:-translate-y-0.5 active:translate-y-0"
              >
                {isSubmitting ? (
                  <>
              ...
              <div className="flex items-center justify-center gap-1.5 text-[11px] text-brand-muted text-center pt-1">
          ...
          <div className="text-center py-4 space-y-5 animate-in zoom-in-95 duration-200">
            <div className="w-16 h-16 rounded-full bg-brand-success/10 text-brand-success flex items-center justify-center mx-auto shadow-sm">
              <CheckCircle2 className="w-9 h-9" />
            </div>

            <div>
              <h3 className="text-2xl font-bold text-brand-dark mb-1.5">
                অর্ডার সফল হয়েছে! 🎉
              </h3>
              <p className="text-sm text-brand-body leading-relaxed max-w-sm mx-auto">
            ...
            <div className="bg-brand-surface border border-brand-border rounded-2xl p-4 text-left space-y-2 text-xs">
            ...
            <div className="pt-2 space-y-2.5">
              <a
                ...
                className="w-full inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20BA5A] text-white font-semibold text-sm py-3 px-4 rounded-xl shadow-sm transition-colors"
              >
              ...
              <button
                type="button"
                onClick={resetAndClose}
                className="w-full text-xs font-semibold text-brand-muted hover:text-brand-dark py-2"
              >
                উইন্ডো বন্ধ করুন
              </button>
            </div>
